...
if [ -z "$INDEXES" ]; then
    java -Xmx512M -jar $SEARCH_UPDATER_JAR --db-host $DB_HOST --db-name $DB_NAME --db-user $DB_USER --db-password "$DB_PASSWORD" --indexes-dir $INDEXES_DIR --replication-access-token $REPLICATION_ACCESS_TOKEN $@
else
    java -Xmx512M -jar $SEARCH_UPDATER_JAR --indexes $INDEXES --db-host $DB_HOST --db-name $DB_NAME --db-user $DB_USER --db-password "$DB_PASSWORD" --indexes-dir $INDEXES_DIR --replication-access-token $REPLICATION_ACCESS_TOKEN $@
fi
